...
public class GetClasificacionExtendidaByCategoria {
	private static final String OBTENER_RESULTADOS = "select c.dorsal, c.tiempoSalida, c.tiempoLlegada, a.sexo, a.nombre, i.categoria, c.minsByKm, c.diferenciaTiempo from Clasificacion c, Atleta a, Inscripcion i where c.idCompeticion = ? and a.email=c.emailAtleta and i.idCompeticion = c.idCompeticion and i.emailAtleta = c.emailAtleta and c.tiempoLlegada is not null and i.categoria=? order by c.tiempoLlegada-c.tiempoSalida";
	...
	private static final String OBTENER_RESULTADOS_DNF = "select c.dorsal, c.tiempoSalida, a.sexo, a.nombre, i.categoria, c.minsByKm, c.diferenciaTiempo from Clasificacion c, Atleta a, Inscripcion i where c.idCompeticion = ? and a.email=c.emailAtleta and i.idCompeticion = c.idCompeticion and i.emailAtleta = c.emailAtleta and c.dorsal is not null and c.tiempoLlegada is null and i.categoria=? order by c.dorsal";
	...
	public GetClasificacionExtendidaByCategoria(CompeticionDto comp, String categoria) {
		this.id_competicion = comp.id;
		this.categoria = categoria;
	}

	public List<ClasificacionDto> execute() {
		List<ClasificacionDto> noPresentados = db.executeQueryPojo(ClasificacionDto.class,
				OBTENER_NO_PRESENTADOS, id_competicion, categoria, id_competicion, categoria);
		for (ClasificacionDto c : noPresentados) {
			c.presentado = false;
		}
		List<ClasificacionDto> clasificados = db.executeQueryPojo(ClasificacionDto.class,
				OBTENER_RESULTADOS, id_competicion, categoria);
		int posicion = 0;
		for (ClasificacionDto c : clasificados) {
			c.posicion = ++posicion;
		}
		List<ClasificacionDto> clasificacion = Stream.concat(Stream.concat(
