...
public class GetClasificacionByCategoria{
	...
	private static final String OBTENER_RESULTADOS_DNS = "select a.sexo, a.nombre, i.categoria, i.clubAtleta from Clasificacion c, Atleta a, Inscripcion i where c.idCompeticion = ? and a.email=c.emailAtleta and i.idCompeticion = c.idCompeticion and i.emailAtleta = c.emailAtleta and c.dorsal is null and i.categoria=?";
	...
	private static final String OBTENER_NO_PRESENTADOS = "select a.sexo, a.nombre, i.categoria,  i.clubAtleta from Clasificacion c, Atleta a, Inscripcion i where i.idCompeticion = ? and a.email=i.emailAtleta and i.idCompeticion = c.idCompeticion and i.categoria=? and i.emailAtleta not in (select cl.emailAtleta from Clasificacion cl, Inscripcion inc where cl.idCompeticion = ? and inc.idCompeticion = cl.idCompeticion and inc.categoria=?) group by i.emailAtleta";
